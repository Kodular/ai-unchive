...
import {INPUT, OUTPUT, TEXT_CATEGORY_HUE, YailTypeToBlocklyType} from "./utilities.ts";
...
Blockly.Blocks['text_join'] = {
  ...
  addInput: function (inputNum) {
    var input = this.appendValueInput(this.repeatingInputName + inputNum).setCheck(YailTypeToBlocklyType("text", INPUT));
    if (inputNum === 0) {
...
Blockly.Blocks['text_starts_at'] = {
  ...
  init: function () {
    this.setColour(TEXT_CATEGORY_HUE);
    this.setOutput(true, YailTypeToBlocklyType("number", OUTPUT));
    var checkTypeText = YailTypeToBlocklyType("text", INPUT);
...
Blockly.Blocks.text_split.adjustToMode = function (mode, block) {
  if (mode === 'SPLITATFIRST' || mode === 'SPLIT') {
    block.getInput("AT").setCheck(YailTypeToBlocklyType("text", INPUT));
    block.setFieldValue(Blockly.Msg.LANG_TEXT_SPLIT_INPUT_AT, 'ARG2_NAME');
  } else if (mode === 'SPLITATFIRSTOFANY' || mode === 'SPLITATANY') {
    block.getInput("AT").setCheck(YailTypeToBlocklyType("list", INPUT));
...
Blockly.Blocks['text_segment'] = {
  ...
  init: function () {
    ...
    this.setOutput(true, YailTypeToBlocklyType("text", OUTPUT));
    var checkTypeText = YailTypeToBlocklyType("text", INPUT);
    var checkTypeNumber = YailTypeToBlocklyType("number", INPUT);
...
Blockly.Blocks['text_replace_mappings'] = {
  ...
  init: function () {
    ...
    this.setOutput(true, YailTypeToBlocklyType("text", OUTPUT));
    var checkTypeText = YailTypeToBlocklyType("text", INPUT);
    var checkTypeMap = YailTypeToBlocklyType("dictionary", INPUT);
