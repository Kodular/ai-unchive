...
import {INPUT, MATH_CATEGORY_HUE, OUTPUT, YailTypeToBlocklyType} from "./utilities.ts";
...
Blockly.Blocks['math_number_radix'] = {
  ...
  init: function() {
    ...
    this.setOutput(true, YailTypeToBlocklyType(
      "number", OUTPUT));
...
Blockly.Blocks.math_compare.onchange = function (value) {
  ...
  if (value === "EQ" || value === "NEQ") {
    ...
    this.sourceBlock_.getInput("A")
      .setCheck(YailTypeToBlocklyType("number", INPUT));
...
Blockly.Blocks['math_multiply'] = {
  ...
  domToMutation: function(container) {
    Blockly.domToMutation.call(this, container);
    if (this.itemCount_ === 1) {
      this.inputList[0].appendField('1 ' + times_symbol);
    }
  },
  decompose: function (workspace) {
    return Blockly.decompose(workspace, 'math_mutator_item', this);
  ...
  addInput: function (inputNum) {
    var input = this.appendValueInput(this.repeatingInputName + inputNum)
      .setCheck(YailTypeToBlocklyType("number", INPUT));
    if (inputNum !== 0) {
      input.appendField(times_symbol);
    }
    return input;
  },
  updateContainerBlock: function (containerBlock) {
    containerBlock.setFieldValue(times_symbol, "CONTAINER_TEXT");
  },
...
Blockly.Blocks['math_random_set_seed'] = {
  ...
  init: function () {
    this.setColour(MATH_CATEGORY_HUE);
    this.setOutput(false, YailTypeToBlocklyType("number", OUTPUT));
